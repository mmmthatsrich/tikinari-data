...
def _db() -> sqlite3.Connection:
    ...
    con.executescript("""
        ...
        CREATE TABLE relation (
            ...
            target_entry_id INTEGER, target_sense_id INTEGER);
    """)
    # entry 1: one sense (10).  entry 2: two senses (20, 21).  entry 3: none.
    con.executemany("INSERT INTO entry (id) VALUES (?)", [(1,), (2,), (3,)])
...
class ResolveUnambiguousSenses(unittest.TestCase):
    def test_a_link_to_a_single_sense_entry_is_resolved(self):
        con = _db()
        con.execute("INSERT INTO ETY_entry_link (id, entry_id) VALUES (1, 1)")
    ...
    def test_a_link_to_a_multi_sense_entry_is_left_for_the_sweep(self):
        con = _db()
        con.execute("INSERT INTO ETY_entry_link (id, entry_id) VALUES (1, 2)")
        resolve_unambiguous_senses(con)
        self.assertIsNone(
            con.execute("SELECT sense_id FROM ETY_entry_link WHERE id=1").fetchone()[0])
    ...
    def test_a_relation_target_with_one_sense_is_resolved(self):
        con = _db()
        con.execute("INSERT INTO relation (id, entry_id, target_entry_id) "
                    "VALUES (1, 2, 1)")
        resolve_unambiguous_senses(con)
        self.assertEqual(
    ...
    def test_an_entry_with_no_senses_resolves_to_nothing(self):
        con = _db()
        con.execute("INSERT INTO ETY_entry_link (id, entry_id) VALUES (1, 3)")
        resolve_unambiguous_senses(con)
        self.assertIsNone(
            con.execute("SELECT sense_id FROM ETY_entry_link WHERE id=1").fetchone()[0])
    ...
    def test_a_sense_already_chosen_is_never_overwritten(self):
        ...
        con = _db()
        con.execute("INSERT INTO ETY_entry_link (id, entry_id, sense_id) "
                    "VALUES (1, 2, 21)")
        resolve_unambiguous_senses(con)
        self.assertEqual(
            con.execute("SELECT sense_id FROM ETY_entry_link WHERE id=1").fetchone()[0],
